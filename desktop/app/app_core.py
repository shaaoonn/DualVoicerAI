"""VoiceTypingApp — the main application class.

Mixin-composed CTk top-level window. Most of the actual behaviour lives
in the mixins under ``app/`` and ``auth/``; this module is the
assembly point — it picks the right MRO, declares the few attributes
that are owned by the constructor itself, and runs the startup
sequence.

Order matters here:

* ``ctk.CTk`` MUST stay FIRST in the bases tuple so ``super().__init__()``
  resolves to it. Mixins have no ``__init__`` of their own.
* The mixin order after that is arbitrary for behaviour but follows the
  refactor sequence (B.5 → B.18) for readability.
"""
...
from app.background_updates import BackgroundUpdateManager
...
# Mixin imports — each contributes a coherent slice of behaviour.
from app.ai_actions import AIActionsMixin
from app.drawers import DrawerMixin
from app.hotkeys import HotkeyMixin
from app.pen_mode import PenModeMixin
from app.screenshot import ScreenshotMixin
from app.settings_io import SettingsIOMixin
from app.tray import TrayMixin
from app.tts import TTSMixin
from app.ui_builder import UIBuilderMixin
from app.update_ui import UpdateUIMixin
from app.voice_pipeline import VoicePipelineMixin
from app.window_chrome import WindowChromeMixin
from auth.auth_panel import AuthPanelMixin
